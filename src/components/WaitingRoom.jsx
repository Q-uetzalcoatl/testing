...
export default function WaitingRoom({ studentName, quizId, appId, db, user, onBack }) {
  const [result, setResult] = useState(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (!studentName || !quizId) return;
    if (!user) return; // Wait for user prop

    const ledgerId = `${user.uid}_${quizId}`;
    const docRef = doc(db, 'artifacts', appId, 'public', 'data', 'class_results', ledgerId);
    ...
    return () => unsub();
  }, [studentName, quizId, appId, db, user]);

  if (loading) return <div className="text-emerald-600 font-bold animate-pulse text-center mt-20">Syncing with server...</div>;
